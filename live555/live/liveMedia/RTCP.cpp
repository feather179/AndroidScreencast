...
void RTCPInstance
::injectReport(u_int8_t const *packet, unsigned packetSize,
          ...
          reportSenderSSRC = fromAddressAndPort.sin_addr.s_addr^portNum(fromAddressAndPort);
            ...
            switch (pt) {
                    ...
                    while (length >= 8) { // A valid chunk must be at least 8 bytes long
                        ...
                        while (itemType != 0) {
                            ...
                            fprintf(stderr, "\t\t%s:%s\n",
                                    ...
                                    "(unknown)",
                                    itemType < 8
                                    ? (char *) pkt // hack, because we know it's '\0'-terminated
                ...
                case RTCP_PT_PSFB: {
                    ...
                    //#####
                    if (length >= 12 && pkt[4] == 'R' && pkt[5] == 'E' && pkt[6] == 'M' &&
                        pkt[7] == 'B') {
...
#ifdef DEBUG
            fprintf(stderr,
                    "validated RTCP subpacket: rc:%d, pt:%d, bytes remaining:%d, report sender SSRC:0x%08x\n",
